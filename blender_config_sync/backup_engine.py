...
class BackupEngine:
    ...
    def create_backup(self, config_path: Path, blender_version: str,
        ...
        # 逻辑：
        # 1. parent 是 X.X 格式？直接用（如 \Blender\5.1\config）
        # 2. grandparent 是 X.X 格式？直接用（如 \Blender\portable\5.1\config）
        # 3. 否则在 parent 的同级目录中找 X.X 格式的文件夹
        parent = config_path.parent
        grandparent = parent.parent
        
        final_version = blender_version
        
        if is_version_dir(parent.name):
            final_version = parent.name
        elif is_version_dir(grandparent.name):
            final_version = grandparent.name
        else:
            # 在 parent 的同级目录中找 X.X 格式的文件夹
            for item in parent.iterdir():
                if item.is_dir() and is_version_dir(item.name):
                    final_version = item.name
                    break
